...
import { Loader2, Delete, ClipboardList, History, LogOut, Package, ArrowLeft } from "lucide-react";
...
export default function FifoPinAccess() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save session
      sessionStorage.setItem("fifo_staff_session", JSON.stringify({
        member: memberData,
  ...
  const handleLogout = () => {
    sessionStorage.removeItem("fifo_staff_session");
  ...
  const handleActionSelect = (action: 'inventory' | 'history') => {
    if (!loggedInWorkspace || !loggedInMember) return;
    
    if (action === 'inventory') {
      navigate(`/inventory-manager?workspace=${loggedInWorkspace.id}&staff=true`);
    } else {
      navigate(`/fifo-activity-log?workspace=${loggedInWorkspace.id}`);
    }
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center relative">
            ...
            <CardTitle className="text-2xl">FIFO Manager</CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            >
              ...
              <div className="text-left">
                ...
                <div className="text-sm text-muted-foreground">Record & manage FIFO items</div>
              </div>
            </Button>
        ...
        <div className="flex-1 flex items-center justify-center">
          <Card className="w-full max-w-md">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-2xl">FIFO Manager</CardTitle>
              <p className="text-muted-foreground mt-2">Inventory Recording System</p>
              <p className="text-sm text-muted-foreground mt-1">Select your workspace to continue</p>
            </CardHeader>
            <CardContent className="space-y-4">
              {workspaces.length === 0 ? (
                <p className="text-center text-muted-foreground py-4">
                  No FIFO workspaces available. Contact your admin.
                </p>
              ) : (
                <Select
                  onValueChange={(value) => {
              ...
              {[0, 1, 2, 3].map(i => (
                <div
                  ...
                  className={`w-12 h-12 rounded-lg border-2 flex items-center justify-center text-2xl font-bold transition-all ${
                    pin.length > i ? 'border-rose-500 bg-rose-500/10' : 'border-muted'
                  }`}
                >
                  {pin.length > i ? '•' : ''}
                </div>
