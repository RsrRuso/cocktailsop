...
const Profile = () => {
  ...
  // Simple verified check - only on mount
  useEffect(() => {
    if (!user?.id) return;
    supabase
      .from('user_roles')
  ...
  // Show skeleton if no cached profile
  ...
  return (
    <div className="min-h-screen bg-background pb-20 pt-16">
        ...
        <div className="glass rounded-xl p-4 border border-border/50">
          <div className="flex items-start gap-4">
            <OptimizedAvatar
              ...
              userId={user.id}
              className={`w-20 h-20 ring-2 ring-offset-2 ring-offset-background bg-gradient-to-br ${getBadgeColor(p.badge_level)}`}
            />
      ...
      {/* Lazy dialogs - only load when opened */}
      {showFollowers && (
