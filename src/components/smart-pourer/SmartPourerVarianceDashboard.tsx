...
import { 
  Scale, TrendingUp, TrendingDown, AlertTriangle, RefreshCw, 
  Download, DollarSign, FileText, CheckCircle2
} from 'lucide-react';
...
const VARIANCE_REASONS = [
  ...
  { value: 'wrong_pairing', label: 'Wrong Pairing' },
...
export function SmartPourerVarianceDashboard({ outletId, outletName }: SmartPourerVarianceDashboardProps) {
  ...
  const calculateVariance = async () => {
    ...
    try {
      ...
      // Fetch pour events (measured usage)
      const { data: pourEvents } = await supabase
        .from('smart_pourer_pour_events')
        .select(`
          poured_ml,
          sku_id,
          sku:smart_pourer_skus(spirit_name, cost_per_ml)
        `)
        .gte('started_at', start.toISOString())
        .lte('started_at', end.toISOString())
        .eq('error_flag', false);

      // Fetch POS sales (expected usage based on recipes)
      const { data: posSales } = await supabase
        .from('smart_pourer_pos_sales')
        .select(`
          quantity,
          pos_recipe_mappings:smart_pourer_pos_recipe_mapping(
            recipe:smart_pourer_recipes(
              recipe_items:smart_pourer_recipe_items(sku_id, ml_required)
            )
          )
        `)
      ...
      // Aggregate measured usage by SKU
        ...
        if (p.sku_id) {
          if (!measuredBySku[p.sku_id]) {
            measuredBySku[p.sku_id] = {
              ml: 0,
              name: p.sku?.spirit_name || 'Unknown',
      ...
      // Aggregate expected usage by SKU from POS sales
      const expectedBySku: Record<string, number> = {};
      (posSales || []).forEach((sale: any) => {
        const mapping = sale.pos_recipe_mappings?.[0];
        const recipe = mapping?.recipe;
        if (recipe?.recipe_items) {
          recipe.recipe_items.forEach((item: any) => {
            if (item.sku_id) {
              expectedBySku[item.sku_id] = (expectedBySku[item.sku_id] || 0) + (item.ml_required * sale.quantity);
            }
          });
        }
      });
      ...
      const allSkuIds = new Set([...Object.keys(measuredBySku), ...Object.keys(expectedBySku)]);
      const variance: VarianceData[] = Array.from(allSkuIds).map(skuId => {
        const measured = measuredBySku[skuId]?.ml || 0;
        const expected = expectedBySku[skuId] || 0;
        const varianceMl = measured - expected;
        const variancePct = expected > 0 ? ((measured - expected) / expected) * 100 : 0;
        const costPerMl = measuredBySku[skuId]?.costPerMl || 0;

        return {
          sku_id: skuId,
          sku_name: measuredBySku[skuId]?.name || 'Unknown',
      ...
      setTotalStats({
        ...totals,
        variancePct: totals.expectedMl > 0 
          ? ((totals.measuredMl - totals.expectedMl) / totals.expectedMl) * 100 
          : 0,
      });
    } catch (error) {
  ...
  const exportReport = () => {
    ...
    a.href = url;
    a.download = `smart-pourer-variance-${format(new Date(), 'yyyy-MM-dd')}.json`;
    a.click();
    ...
    <div className="space-y-4">
      ...
      <Card className={`border-2 ${overallStatus.color.replace('bg-', 'border-')}/50`}>
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="text-center">
              <p className="text-sm text-muted-foreground">Expected (SOP)</p>
        ...
        <Card className="bg-muted/30">
          <CardContent className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-muted-foreground text-center">
              No variance data for this period.<br />
              Record pours and sales to see variance analysis.
            </p>
          </CardContent>
        </Card>
      ) : (
      ...
      <Dialog open={isExplainDialogOpen} onOpenChange={setIsExplainDialogOpen}>
        <DialogContent>
            ...
            <div className="p-3 bg-muted/30 rounded-lg">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-red-500">
                  {selectedVariance?.variance_ml.toFixed(0)}ml ({selectedVariance?.variance_pct.toFixed(1)}%)
                </span>
              </div>
              <div className="flex justify-between text-sm mt-1">
                <span>Cost Impact:</span>
                <span className="font-bold text-red-500">
                  ${selectedVariance?.variance_cost.toFixed(2)}
                </span>
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsExplainDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleSubmitExplanation} disabled={!reason}>
