...
interface InviteFifoMemberDialogProps {
  open: boolean;
...
export const InviteFifoMemberDialog = ({
  open,
  onOpenChange,
  workspaceId,
  workspaceName,
  onSuccess,
}: InviteFifoMemberDialogProps) => {
  const [status, setStatus] = useState<"member" | "admin">("member");
    ...
    try {
      ...
      const members = Array.from(selectedUsers).map(userId => ({
        ...
        permissions: {
          can_receive: true,
          can_transfer: true,
          can_manage: status === "admin",
          can_delete: status === "admin"
        },
        invited_by: user.id,
      }));
      ...
      toast.success(`Added ${selectedUsers.size} member(s) to FIFO workspace`);
      setSelectedUsers(new Set());
      onSuccess?.();
      onOpenChange(false);
    } catch (error: any) {
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 sm:px-6 py-3 space-y-3 border-b shrink-0">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs sm:text-sm">Status for selected members</Label>
              <Select
                value={status}
                onValueChange={(value: "member" | "admin") => setStatus(value)}
                disabled={isLoading}
              >
                ...
                <SelectContent>
                  <SelectItem value="member">Member (Can receive & transfer)</SelectItem>
